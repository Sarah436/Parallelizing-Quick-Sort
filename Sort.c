...
void main(int argc, char *argv[]){
        ...
        MPI_Status status; //MPI _Status represents the status of a reception operation, returned by receive operations (MPI_Recv), non-bloking
        ...
        /*...MPI Initialisation...*/
        //The functions MPI INIT and MPI FINALIZE are used to initiate and shutdown an MPI computation
        MPI_Init(&argc, &argv);

        // A barrier can be used to synchronize all processes in communicator.
        MPI_Barrier (MPI_COMM_WORLD) ;
        // MPI Wtime returns a floating-point number of seconds, representing elapsed wall-clock time since some time in the past.
       ...
       if(myid == Root) {
        ...
        MPI_Scatter(array,local_n,MPI_LONG,local_a,local_n,
        ...
        if(myid==Root){                         
         ...
         if (myid != Root){
                  MPI_Send(&local_a, local_n, MPI_LONG, Root, myid, 
                  MPI_COMM_WORLD);
          }
         if (myid == Root) {
          ...
          if(myid == Root){
                  ...
                  while (count <n){
                           ...
                           for (int i=0;i<numprocs;i++){
                                    if(iterators[i]<local_n && 
                                       minVal>=arrayGather[i]
                                         [iterators[i]]){minVal= arrayGather[i]  
                                             [iterators[i]];
                                            procNo=i;
                                     }
                            }
                            if(iterators[numprocs]<remaining_length &&  
                                remained[iterators[numprocs]]<minVal){
                                  array[count] =  
                                   remained[iterators[numprocs]];
                                   iterators[numprocs]++;
                            }
                            else{
                                    array[count]=minVal;
